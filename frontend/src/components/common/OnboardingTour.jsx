...
export default function OnboardingTour() {
  ...
  const hasMemberships = Boolean(userMemberships?.data?.length);
  ...
  useEffect(() => {
    ...
    setCurrentStep((prev) => (prev === initialStep ? prev : initialStep));
    setIsVisible(true);
  }, [isSignedIn, orgId, hasMemberships]);
  ...
  useEffect(() => {
    if (isVisible && currentStep === 0 && orgId && hasMemberships) {
  ...
  }, [orgId, hasMemberships, isVisible, currentStep]);
