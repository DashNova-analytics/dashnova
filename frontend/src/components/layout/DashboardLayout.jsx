...
export default function DashboardLayout({ children }) {
  ...
  const notifications = [
    { id: 1, text: 'New business data successfully compiled.', time: '5m ago' },
    { id: 2, text: 'Weekly revenue projection ready for review.', time: '2h ago' },
    { id: 3, text: 'AI Assistant: Seasonal product recommendation created.', time: '1d ago' },
  ];
    ...
    <div className="min-h-screen bg-white flex text-gray-900 font-sans antialiased">
      ...
      >
        ...
        <div className="h-16 border-b border-gray-200 px-4 flex items-center justify-between">
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="lg:hidden text-gray-500 hover:text-black focus:outline-none cursor-pointer"
          >
            <X size={16} />
          </button>
        </div>
      ...
      >
        {/* Top Navbar */}
        <header className="h-16 border-b border-gray-200 bg-white sticky top-0 z-30 px-6 md:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4 flex-1">
            ...
            {/* Organization Switcher (module) */}
            <OrganizationSwitcher
              hidePersonal={false}
              afterCreateOrganizationUrl="/dashboard"
              afterSelectOrganizationUrl="/dashboard"
            />
            {/* Context-aware Back button (Fix 4) */}
            {location.pathname !== '/dashboard' && (
              <button
                onClick={() => navigate(-1)}
                className="flex items-center gap-1.5 text-xs font-bold text-gray-500 hover:text-black hover:bg-gray-100/60 px-2.5 py-1.5 rounded transition cursor-pointer border border-gray-100 hover:border-gray-200 focus:outline-none shrink-0"
              >
                <ChevronLeft size={13} />
                <span>Back</span>
              </button>
            ...
            <div className="relative hidden md:flex items-center w-full max-w-md">
              ...
              <input
                ...
                className="w-full bg-gray-50 border border-gray-200 rounded-md py-1.5 pl-9 pr-4 text-xs focus:outline-none focus:ring-1 focus:ring-gray-300"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <button
                ...
                className="w-8 h-8 rounded border border-gray-200 hover:border-gray-400 text-gray-500 hover:text-black flex items-center justify-center relative transition focus:outline-none cursor-pointer bg-white"
              >
                <Bell size={14} />
                <span className="absolute top-1.5 right-2 w-1.5 h-1.5 bg-black rounded-full" />
              </button>
              ...
              {showNotifications && (
                <>
                  ...
                  />
                  <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded shadow-lg py-2 z-50 animate-fade-in font-sans">
                    <div className="px-4 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">
                      Recent System Notifications
                    </div>
                    <div className="divide-y divide-gray-100 max-h-60 overflow-y-auto">
                      {notifications.map((n) => (
                        <div key={n.id} className="p-3 hover:bg-gray-50 transition cursor-pointer">
                          <p className="text-xs text-gray-800 leading-normal">{n.text}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
              ...
              <div>
                ...
                <ul className="space-y-2 text-xs text-gray-600">
                  {['Dashboard', 'Analytics', 'Forecasting', 'Reports'].map(item => (
                    <li key={item}>
