...
export default function UploadZone({ onUploadComplete, allowedTypes = ['.csv', '.xlsx', '.xls'] }) {
  ...
  const [statusStage, setStatusStage] = useState('Uploading spreadsheet...');
  ...
  const processFile = async (selectedFile) => {
    ...
    setUploading(true);
    setProgress(15);
    setStatusStage('Reading file structure and headers...');

    try {
      await new Promise((resolve) => setTimeout(resolve, 200));
      setProgress(45);
      setStatusStage('Extracting rows, revenue totals, & dates...');

      await new Promise((resolve) => setTimeout(resolve, 250));
      setProgress(80);
      setStatusStage('Generating KPIs, charts, & metrics...');

      await new Promise((resolve) => setTimeout(resolve, 200));
      setProgress(100);
      setStatusStage('Extraction complete! Rendering analytics...');
      ...
      >
        ...
        {uploading ? (
          <div className="w-full max-w-xs space-y-4">
            ...
            <div>
              <p className="text-xs font-semibold text-gray-950">{statusStage}</p>
