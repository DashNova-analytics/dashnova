...
export default function OrganizationProfile() {
  ...
  const handleDeleteOrg = async () => {
    if (
      !window.confirm(
        `Delete "${organization.name}" permanently? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      // Destroy the org — will automatically clear the active org
      await organization.destroy();

      toast.success("Organization deleted.");

      // Give a moment to re-issue the session token before navigating.
      // Without this delay, the route guards see userId as null during
      // token refresh and trigger a redirect loop.
      setTimeout(() => {
  ...
  return (
    <div className="w-full border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm flex flex-col md:flex-row min-h-[480px]">
