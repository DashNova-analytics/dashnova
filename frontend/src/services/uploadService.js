...
export const uploadService = {
   ...
   */
  uploadFile: async (file, onProgress, organizationId) => {
    ...
    // Save newly uploaded file record locally
    ...
    // Also attempt to persist upload metadata to backend database
    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('metadata', JSON.stringify({ recordsDiscovered: rowCount }));
      if (organizationId) {
        formData.append('organizationId', organizationId);
      }
      await api.post('/uploads', formData);
    } catch (backendErr) {
      console.warn('Backend upload persistence warning:', backendErr.message);
    }
