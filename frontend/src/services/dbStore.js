 ...
 */
export const syncDatabaseWithFiles = async (files, organizationName) => {
  ...
  const monthOrderMap = new Map(); // 'Jan' → orderCount
  ...
  for (const row of allRows) {
    ...
    if (rowDate) {
      ...
      monthOrderMap.set(monthKey, (monthOrderMap.get(monthKey) || 0) + 1);
  ...
  if (monthRevMap.size > 0) {
    ...
    for (const month of orderedMonths) {
      ...
      const mRev = Math.round(monthRevMap.get(month) * 100) / 100;
      const mOrders = monthOrderMap.get(month) || 1;
      revenueOverTime.push({
        month,
        revenue: mRev,
        sales: mOrders,
        target: Math.round(mRev * 0.9 * 100) / 100,
  ...
  for (const [name, prod] of productMap.entries()) {
    products.push({
      ...
      price: prod.qty > 0
        ? `Rs ${(prod.revenue / prod.qty).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
        : '—',
      soldCount: prod.qty,
      revenue: `Rs ${prod.revenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
    });
    skuIndex++;
  }
  ...
  for (const [, cust] of customerMap.entries()) {
    customers.push({
      ...
      name: cust.name,
      email: cust.email,
      sales: `Rs ${cust.revenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      orders: cust.orders,
      region: cust.region,
  ...
  if (totalRevenue > 0) {
    aiInsights.push({
      ...
      detail: `Total revenue across ${salesCount.toLocaleString()} transactions: Rs ${totalRevenue.toLocaleString(undefined, { maximumFractionDigits: 2 })}. Average order value: Rs ${averageOrderValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}.`,
    });
  }
  ...
  if (regionalAnalytics.length > 0) {
    ...
    aiInsights.push({
      title: 'Top Region',
      detail: `${topRegion.region} accounts for ${regionPercent}% of total revenue (Rs ${topRegion.sales.toLocaleString(undefined, { maximumFractionDigits: 2 })}).`,
    });
  }
  ...
  // ─── Health Score & Pillars Calculation ──────────────────────────
  const revenueScore = totalRevenue > 0 ? 88 : 50;
  const profitScore = averageOrderValue > 0 ? 90 : 50;
  const growthScore = revenueOverTime.length > 1 ? (revenueOverTime[revenueOverTime.length - 1].revenue >= revenueOverTime[0].revenue ? 85 : 60) : 50;
  const customerScore = activeCustomers > 0 ? 92 : 50;
  const stockScore = stockVelocity.length > 0 ? (stockVelocity.some(s => s.depletionDays < 10) ? 48 : 82) : 75;
  const cashFlowScore = totalRevenue > 0 ? 85 : 50;

  const compositeHealthScore = Math.round((revenueScore + profitScore + growthScore + customerScore + stockScore + cashFlowScore) / 6);

  const healthPillars = [
    { name: 'Revenue', status: revenueScore >= 80 ? 'check' : 'neutral', score: revenueScore, desc: totalRevenue > 0 ? `Rs ${totalRevenue.toLocaleString()} tracked` : 'No revenue data' },
    { name: 'Profit', status: profitScore >= 80 ? 'check' : 'neutral', score: profitScore, desc: averageOrderValue > 0 ? `Avg order: Rs ${averageOrderValue.toFixed(2)}` : 'No order data' },
    { name: 'Growth', status: growthScore >= 80 ? 'check' : 'neutral', score: growthScore, desc: revenueOverTime.length > 1 ? `${revenueOverTime.length} months tracked` : 'Single period' },
    { name: 'Customer', status: customerScore >= 80 ? 'check' : 'neutral', score: customerScore, desc: `${activeCustomers} unique accounts` },
    { name: 'Inventory', status: stockScore < 60 ? 'alert' : 'check', score: stockScore, desc: stockVelocity.length > 0 ? `${stockVelocity.length} SKUs tracked` : 'No stock velocity data' },
    { name: 'Cash Flow', status: cashFlowScore >= 80 ? 'check' : 'neutral', score: cashFlowScore, desc: totalRevenue > 0 ? 'Positive ledger flow' : 'Awaiting ledger data' },
  ];
  ...
  if (totalRevenue > 0) {
    alerts.push({
      id: 'a1',
      type: 'success',
      title: 'Ledger Ingestion Synchronized',
      message: `Ingested ${salesCount.toLocaleString()} invoice items generating Rs ${totalRevenue.toLocaleString(undefined, { maximumFractionDigits: 2 })} total revenue.`,
  ...
  if (products.length > 0) {
    alerts.push({
      ...
      message: `"${products[0].name}" is your highest performer with ${products[0].revenue} revenue.`,
      actionText: 'View Catalog',
      actionKey: 'product',
      time: 'Just now'
    });
  }
  if (stockVelocity.length > 0 && stockVelocity[0].depletionDays < 15) {
    alerts.push({
      id: 'a3',
      type: 'warning',
      title: 'Stock Velocity Warning',
      message: `SKU "${stockVelocity[0].name}" estimated depletion in ~${stockVelocity[0].depletionDays} days.`,
      actionText: 'Restock Alert',
      actionKey: 'restock',
      time: 'Just now'
    });
  }

  // ─── Dynamic Analyst Briefing ──────────────────────────────────────
  const analystBriefing = {
    keyChanges: [
      totalRevenue > 0 ? `Parsed ${salesCount.toLocaleString()} sales lines generating Rs ${totalRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 })} gross revenue.` : 'No sales records.',
      activeCustomers > 0 ? `Identified ${activeCustomers.toLocaleString()} unique customers across transaction logs.` : 'No customer records.',
      products.length > 0 ? `Catalog compiled with ${products.length} distinct items led by "${products[0].name}".` : 'No product listings.'
    ],
    risks: [
      stockVelocity.length > 0 && stockVelocity[0].depletionDays < 15
        ? `Stock depletion alert on "${stockVelocity[0].name}" in ~${stockVelocity[0].depletionDays} days.`
        : 'Ensure continuous dataset updates for real-time stock risk monitoring.'
    ],
    opportunity: products.length > 1
      ? `Promote "${products[0].name}" alongside "${products[1].name}" to increase average order value above current Rs ${averageOrderValue.toFixed(0)}.`
      : 'Upload multi-period datasets to unlock cross-sell bundle opportunities.'
  };

  // ─── Dynamic Goals ────────────────────────────────────────────────
  const targetRevenueVal = Math.round((totalRevenue || 100000) * 1.25);
  const goals = [
    {
      id: 'g1',
      title: 'Gross Revenue Target',
      target: `Rs ${targetRevenueVal.toLocaleString()}`,
      current: `Rs ${totalRevenue.toLocaleString()}`,
      progress: Math.min(100, Math.round((totalRevenue / (targetRevenueVal || 1)) * 100)),
      status: totalRevenue >= targetRevenueVal ? 'Target Achieved' : 'In Progress',
      estimate: `Based on current Rs ${averageOrderValue.toFixed(0)} average order value`,
      type: 'revenue'
    },
    {
      id: 'g2',
      title: 'Active Customer Target',
      target: `${Math.round((activeCustomers || 10) * 1.3)} accounts`,
      current: `${activeCustomers} accounts`,
      progress: Math.min(100, Math.round((activeCustomers / (Math.round((activeCustomers || 10) * 1.3))) * 100)),
      status: 'In Progress',
      estimate: 'Tracked across uploaded billing ledgers',
      type: 'customers'
    }
  ];
  ...
  const newState = {
    ...
    uploadedFiles: files,
    kpis: {
      totalRevenue: `Rs ${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}`,
      salesCount: salesCount.toLocaleString(),
      averageOrderValue: `Rs ${averageOrderValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
  ...
  // Synchronize parsed products & customers directly into Neon database
  await api.post('/uploads/sync', { products, customers, organizationName });
